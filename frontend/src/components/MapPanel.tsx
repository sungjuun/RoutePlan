...
export function MapPanel({ trip, itinerary, places = {}, compact = false }: Props) {
  ...
  return (
    <div className={`map-panel ${compact ? 'map-panel-compact' : ''}`}>
      ...
      {itinerary && <div className="map-mode-switch"><button type="button" onClick={() => setRoadMode(false)} aria-pressed={!roadMode}>방문 순서 보기</button><button type="button" onClick={() => setRoadMode(true)} aria-pressed={roadMode}>실제 도로 경로</button></div>}
      {roadMode && itinerary ? <GoogleRoadMap itinerary={itinerary} date={selectedDate ?? trip.startDate} /> : <MapContainer
