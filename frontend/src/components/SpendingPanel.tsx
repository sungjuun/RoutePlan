...
export function SpendingPanel({ trip, onError }: { trip: Trip; onError: (e: unknown) => void }) {
  ...
  return <section className="advanced-panel spending-panel" aria-label="예산과 실제 지출">
    ...
    <div className="advanced-list">{value.expenses.map(entry => <article key={entry.id}><div><strong>{entry.description}</strong><small>{entry.date} · {categoryNames[entry.category]}{entry.placeName ? ` · ${entry.placeName}` : ''} · {entry.payerNickname} 결제{entry.participantCount > 1 ? ` · ${entry.participantCount}명 공동 정산` : ''} · {moneyLabel(entry.amountMinor, value.currency)}</small></div><button disabled={busy || entry.participantCount > 1} title={entry.participantCount > 1 ? '공동 정산 화면에서 관리하세요.' : undefined} onClick={() => edit(entry)}>수정</button><button disabled={busy} onClick={() => { if (window.confirm('이 지출 기록을 삭제할까요?')) void run(() => advanced.deleteExpense(trip.id, entry.id)) }}>삭제</button></article>)}{!value.expenses.length && <p>아직 기록한 지출이 없습니다.</p>}</div>
