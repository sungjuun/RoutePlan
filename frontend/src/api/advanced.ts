...
export interface Expense { id: number; requestId: string; date: string; category: ExpenseCategory; description: string; amountMinor: number; placeId: number | null; placeName: string | null; payerUserId: number | null; payerNickname: string; participantCount: number }
...
export const advanced = {
  ...
  expense: (id: number, currency: BudgetCurrency, value: Pick<Expense, 'requestId' | 'date' | 'category' | 'description' | 'amountMinor' | 'placeId'>, expenseId?: number) => send<Spending>(`/trips/${id}/spending/expenses${expenseId ? `/${expenseId}` : ''}`, expenseId ? 'PUT' : 'POST', { ...value, currency }),
