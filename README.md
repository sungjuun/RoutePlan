...
- Google 정규 영업시간 자동 조회·지원 불가 시간창 경고
...
영업시간 정보가 없는 장소는 하루 운영시간 전체에 방문할 수 있는 것으로 취급합니다. 명시적으로 `closed=true`인 날은 방문할 수 없습니다. V17부터 Google 장소는 계산 시 정규 주간 영업시간을 조회하며 수동 설정이 우선합니다. 원본 응답은 영구 저장하지 않습니다. 한 장소에 요일별 영업 구간 하나만 지원하며 분할/야간 영업과 휴일 예외는 경고 후 자동 적용하지 않습니다.
...
- 날씨 자동 조회는 버튼 실행 방식이고 최대 16일 예보만 지원합니다. 백그라운드 정기 갱신은 없으며 무료 Open-Meteo endpoint는 비상업용입니다.
