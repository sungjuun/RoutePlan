...
V20은 [닉네임 변경·이메일 변경 후 재인증·회원 탈퇴와 개인 데이터 정리](docs/account-management.md)를 마이페이지에 추가합니다.

V21은 [외부 API 품질·비용 운영 대시보드](docs/provider-operations.md)를 추가합니다. Google·OpenAI의 재시도별 시도/성공/실패, 지연시간, OpenAI 입출력 토큰과 앱 안전 한도를 PostgreSQL에 보존하며 실제 계약 단가를 설정한 경우에만 USD 추정액을 표시합니다.
...
V25는 [시간대별 교통량 전역 최적화와 PostGIS 영속 경로 캐시](docs/time-dependent-routing.md)를 추가합니다. 비용·탐색 상한 안에서 날짜와 방문 순서를 함께 재탐색하고, Redis L1 miss를 PostGIS L2에서 복구하며 동일 동시 miss는 분산 잠금으로 병합합니다.

설정과 사용법은 [실제 데이터·개인화·장부 안내](docs/advanced-integrations.md), 외부 API 검증 결과는 [실호출 점검 기록](docs/live-validation-2026-08-28.md)을 참고하세요. **2026-09-01 로컬 환경에서 미래 날짜의 Google `TRAFFIC_AWARE` 자동차 Matrix, 시간 의존 전역 탐색, Redis L1 재사용과 PostGIS L2 재가열까지 실제 응답으로 확인했습니다.** 대표 표본 검증이며 실제 청구액·무료 잔량·운영 환경 검증은 별도입니다. 다른 환경에서는 서버·브라우저 키와 해당 API 활성화가 필요합니다.

## 구현 범위 (V1–V25)

### 지원

- Open-Meteo 자동 날씨 조회·수동 예보 보존·여행지 IANA 시간대
- Google 분할·야간 영업 구간 및 제공된 7일 특별 영업시간·휴무 반영
- 각 방문 종료 시각의 대중교통 재검증, 여행별 3시간 주기 예보 갱신
- 마이페이지 프로필 사진 업로드·초기화와 헤더 벨 알림함
- 여행일/현지 출발시각 기준 Transit Matrix와 날짜 없는 캐시 재사용 차단
- Google 지도 전용 실제 도로 Geometry와 선택 날짜 미완료 구간 표시
- Google·OpenAI 호출 시도/성공/실패·지연시간·토큰 집계와 원자적 월별 앱 한도
- 경고/차단 상태·잔여량과 선택적 계약 단가 기반 USD 추정 대시보드
- 날짜별·항목별 예산 한도와 실제 지출 작성·수정·삭제·멱등 저장
...
기본 일정 지도는 숙소와 방문 장소 Marker를 표시하고 방문 순서를 선으로 연결합니다. V17의 ‘실제 도로 경로’로 전환하면 Google 지도와 Polyline을 조회합니다(별도 키 필요). 일정의 거리/시간은 백엔드 Route Matrix 결과이며 지도 조회만으로 다시 계산하지 않습니다. 기본 Place Provider가 `DISABLED`여도 좌표 등록 화면으로 전체 흐름을 실행할 수 있습니다.
...
영업시간 정보가 없는 장소는 하루 운영시간 전체에 방문할 수 있는 것으로 취급합니다. 명시적으로 `closed=true`인 날은 방문할 수 없습니다. V18은 Google 분할 영업을 개별 구간으로, 야간 영업을 날짜별 구간으로 나눕니다. 체류 전체가 한 영업 구간 안에 들어가야 하며 휴식 시간을 가로지를 수 없습니다. 제공된 `currentOpeningHours`의 현지 오늘~6일 뒤는 특별 영업시간·휴무를 우선하고, 그 밖에는 정규 주간 시간을 사용합니다. 기존 수동 설정이 우선하며 원본 응답은 영구 저장하지 않습니다. 상세 범위는 [정확도·프로필·알림 안내](docs/schedule-accuracy-profile.md)를 참고하세요.
...
공개 Route 조회·회원가입/로그인·인증 상태 확인·이메일 링크 인증/비밀번호 복구를 제외한 업무 API는 로그인이 필요합니다. 클라이언트가 `userId`를 보내더라도 소유자는 요청 본문이 아니라 서버 세션의 사용자로 결정됩니다. 공개 복구 API를 포함한 모든 상태 변경 요청은 먼저 CSRF 토큰을 조회해 서버가 알려준 헤더에 전달해야 합니다. 계정 보안과 관리 API는 [V19 안내](docs/account-security.md) 및 [V20 안내](docs/account-management.md)를 참고하세요.
...
| `PATCH` | `/api/v1/auth/profile` | 닉네임 변경 |
| `POST` | `/api/v1/auth/email/change` | 현재 비밀번호 확인 후 이메일 변경·모든 세션 종료 |
| `DELETE` | `/api/v1/auth/account` | 확인 문구·현재 비밀번호 확인 후 계정과 개인 데이터 삭제 |
...
- Redis 장애 시 PostGIS 조회·재가열, PostGIS 장애 시 Google·Redis fallback
- 두 Provider 인스턴스의 동일 동시 miss를 Redis 분산 잠금으로 단일 Google 호출로 병합
- 4·6·8개 장소와 Beam 128·256·512 조합의 시간 의존 탐색 성능·품질 비교
...
V19 이메일 인증·비밀번호 복구/변경·로그인 제한·PostgreSQL 영속 세션 ✓
 ↓
V20 닉네임·이메일 변경·재인증·회원 탈퇴·개인 데이터 정리 ✓
 ↓
...
- 로컬 대표 표본으로 날씨·영업시간·도보·대중교통·미래 자동차 교통량 응답, Google 지도 표시와 Redis/PostGIS 계층 캐시를 확인했습니다. 모든 국가·이동수단·날짜 조합의 검증은 아니며, 실제 청구액/무료 잔량과 운영 호출 한도는 Cloud Billing·Console에서 별도 확인해야 합니다.
...
- 기본 지도 선은 방문 순서 시각화입니다. 별도 브라우저 키를 설정하면 Google 지도에서 실제 도로 Geometry를 조회할 수 있으며, 이전 일정은 새 버전 계산이 필요합니다.
- 다일 장소 배분은 날짜 순차 휴리스틱이며 날짜·장소 조합의 전역 최적해를 보장하지 않습니다.
- 다일 재최적화도 날짜 순차 휴리스틱을 사용하며, 현재 날짜의 완료 상태는 기준 일정의 연속된 앞부분만 지원합니다.
- 날씨는 최대 16일 예보를 지원하며, 여행별로 켠 정기 갱신은 서버 실행 중에만 동작합니다. 무료 Open-Meteo endpoint는 비상업용입니다.
...
- 비용과 실제 지출은 사용자가 입력합니다. 실시간 입장료·교통요금, 환율 변환, 인원별 자동 곱셈, 결제 연동·인원별 정산은 지원하지 않습니다.
...
- 로그인 세션은 Spring Session JDBC로 PostgreSQL에 공유합니다. 다중 인스턴스 운영 전에는 실제 부하에서 세션 정리 주기·DB 인덱스·연결 풀 용량을 검증해야 합니다.
...
- OpenAI의 앱 내 토큰·지연시간·추정 비용은 응답 시점부터 집계합니다. 실제 청구액·계정 잔액·다른 앱 사용량은 공급자 대시보드에서 별도 확인해야 합니다.
