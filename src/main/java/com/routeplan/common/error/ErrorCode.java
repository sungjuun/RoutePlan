...
public enum ErrorCode {
    ...
    AUTH_RATE_LIMITED(HttpStatus.TOO_MANY_REQUESTS, "요청이 너무 많습니다. 잠시 후 다시 시도해 주세요."),
    AUTH_TOKEN_INVALID(HttpStatus.BAD_REQUEST, "유효하지 않거나 만료된 링크입니다. 새 메일을 요청해 주세요."),
    AUTH_MAIL_DISABLED(HttpStatus.SERVICE_UNAVAILABLE, "이메일 발송이 설정되지 않았습니다. 관리자에게 문의해 주세요."),
    ...
    DUPLICATE_WISHLIST(HttpStatus.CONFLICT, "같은 이름의 위시리스트가 이미 있습니다."),
    DUPLICATE_WISHLIST_PLACE(HttpStatus.CONFLICT, "위시리스트에 이미 저장된 장소입니다."),
    WISHLIST_PLACE_NOT_FOUND(HttpStatus.NOT_FOUND, "위시리스트에 저장된 장소를 찾을 수 없습니다."),
    WISHLIST_PLACE_LIMIT_EXCEEDED(HttpStatus.UNPROCESSABLE_CONTENT, "한 위시리스트에는 장소를 최대 100개까지 저장할 수 있습니다."),
    TRIP_MEMBER_NOT_FOUND(HttpStatus.NOT_FOUND, "여행 동행자를 찾을 수 없습니다."),
    TRIP_MEMBER_ALREADY_EXISTS(HttpStatus.CONFLICT, "이미 이 여행에 참여 중인 사용자입니다."),
    TRIP_OWNER_ROLE_IMMUTABLE(HttpStatus.CONFLICT, "여행 소유자의 권한은 변경하거나 삭제할 수 없습니다."),
    TRIP_MEMBER_LIMIT_EXCEEDED(HttpStatus.UNPROCESSABLE_CONTENT, "한 여행에는 소유자를 포함해 최대 20명까지 참여할 수 있습니다."),
    TRIP_VOTE_NOT_FOUND(HttpStatus.NOT_FOUND, "장소 투표를 찾을 수 없습니다."),
    SETTLEMENT_EXPENSE_NOT_FOUND(HttpStatus.NOT_FOUND, "정산 지출을 찾을 수 없습니다."),
    SETTLEMENT_PARTICIPANT_INVALID(HttpStatus.UNPROCESSABLE_CONTENT, "결제자와 분담자는 현재 여행 동행자여야 합니다."),
    CONTENT_IMPORT_NOT_FOUND(HttpStatus.NOT_FOUND, "가져오기 작업을 찾을 수 없습니다."),
    CONTENT_IMPORT_NOT_READY(HttpStatus.CONFLICT, "아직 저장할 수 없는 가져오기 작업입니다."),
    CONTENT_IMPORT_CANDIDATE_NOT_FOUND(HttpStatus.NOT_FOUND, "선택한 장소 후보를 찾을 수 없습니다."),
    CONTENT_IMPORT_UNSUPPORTED_URL(HttpStatus.BAD_REQUEST, "지원하지 않거나 안전하지 않은 URL입니다."),
    ...
    INFEASIBLE_BUDGET(HttpStatus.UNPROCESSABLE_CONTENT, "필수 비용이 여행 예산을 초과합니다."),
    COST_ESTIMATES_REQUIRED(HttpStatus.UNPROCESSABLE_CONTENT, "예산 적용에 필요한 장소 비용이 입력되지 않았습니다."),
    BUDGET_CURRENCY_MISMATCH(HttpStatus.UNPROCESSABLE_CONTENT, "완료 구간의 통화와 새 예산 통화가 다릅니다. 기존 통화를 사용하거나 전체 일정을 새로 계산해 주세요."),
    ...
    INVALID_MANUAL_ITINERARY(HttpStatus.UNPROCESSABLE_CONTENT, "직접 편집한 일정의 날짜 또는 순서가 올바르지 않습니다."),
    MANUAL_ITINERARY_INFEASIBLE(HttpStatus.UNPROCESSABLE_CONTENT, "선택한 순서는 영업시간과 하루 종료 조건을 만족하지 못합니다."),
